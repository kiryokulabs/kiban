...
| **Wildcard Domain** | Default base domain for installed service URLs | `apps.example.com` |
...
Examples with Wildcard Domain `apps.example.com`:

```txt
grafana.development.crossmetrics.apps.example.com
n8n.production.myproject.apps.example.com
postgresql.staging.crm.apps.example.com
```
...
The Instance Domain replaces `IP:8080` for dashboard access.
...
- Clearing the Instance Domain removes the Traefik labels and stops routing.
...
```txt
n8n.development.crossmetrics.apps.example.com
```
...
```txt
A   *.apps.example.com   →   your VPS IP
```
