...
Kiban owns HTTP routing through the `ProxyProvider` abstraction. The current infrastructure adapter is `TraefikProxyProvider`; future application runtimes can use the same contract without knowing Traefik.
...
- configures HTTPS routers with the `letsencrypt` certificate resolver and redirects HTTP to HTTPS
