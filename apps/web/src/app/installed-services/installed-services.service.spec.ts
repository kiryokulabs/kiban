...
describe('InstalledServicesService', () => {
  it('lists all installed services with credentials', () => { const { http, calls } = createHttpClient(); new InstalledServicesService(http).listAll().subscribe(); expect(calls).toEqual([{ method: 'GET', url: 'http://localhost:3000/services', options: { withCredentials: true } }]); });
