...
describe('InstalledServicesService', () => {
  it('lists all installed services with credentials', () => { const { http, calls } = createHttpClient(); new InstalledServicesService(http).listAll().subscribe(); expect(calls).toEqual([{ method: 'GET', url: '/api/services', options: { withCredentials: true } }]); });
  it('installs services in an environment with credentials', () => { const { http, calls } = createHttpClient(); const service = new InstalledServicesService(http); const body = { serviceId: 'postgresql', configuration: {} }; service.install('project-1', 'env-1', body).subscribe(); expect(calls).toEqual([{ method: 'POST', url: '/api/projects/project-1/environments/env-1/services', body, options: { withCredentials: true } }]); });
  it('starts services through generic service endpoint', () => { const { http, calls } = createHttpClient(); new InstalledServicesService(http).start('service-1').subscribe(); expect(calls).toEqual([{ method: 'PATCH', url: '/api/services/service-1/start', body: {}, options: { withCredentials: true } }]); });
});
