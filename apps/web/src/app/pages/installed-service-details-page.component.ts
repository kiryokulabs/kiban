  ...
  template: `
    <div class="min-w-0 space-y-6">
      <div class="flex flex-wrap items-start justify-between gap-3">
        <div class="min-w-0">
          ...
          <div class="mt-3 flex min-w-0 items-start gap-3">
            ...
            <div class="min-w-0">
              <h1 class="truncate text-xl font-semibold kb-text">{{ details()?.overview?.name ?? 'Service' }}</h1>
              <p class="break-words text-sm c-muted">{{ details()?.overview?.description ?? 'Manage access, configuration, runtime actions and logs.' }}</p>
            </div>
          </div>
        </div>
      ...
      @if (loading() && !details()) {
        ...
        <section class="grid grid-cols-[minmax(0,1fr)] gap-3 sm:grid-cols-2 xl:grid-cols-5">
          <div class="card min-w-0 p-4 xl:col-span-2">
            <p class="flex items-center gap-1.5 text-xs c-muted"><kiban-icon name="folder" [size]="12" /> Location</p>
            <p class="mt-1 break-words text-sm font-medium kb-text">{{ presenter.locationLabel(d) }}</p>
            <p class="mt-1 break-words text-xs c-muted">Project: {{ d.location.project.name }} · Environment: {{ d.location.environment.name }}</p>
          </div>
          <div class="card min-w-0 p-4"><p class="flex items-center gap-1.5 text-xs c-muted"><kiban-icon name="info" [size]="12" /> Status</p><p class="mt-1"><span class="badge" [class.badge-success]="d.overview.status === 'running'" [class.badge-danger]="d.overview.status === 'stopped'" [class.badge-warning]="d.overview.status !== 'running' && d.overview.status !== 'stopped'">{{ d.overview.status }}</span></p></div>
          <div class="card min-w-0 p-4"><p class="flex items-center gap-1.5 text-xs c-muted"><kiban-icon name="check" [size]="12" /> Health</p><p class="mt-1"><span class="badge" [class.badge-success]="d.overview.health === 'healthy'" [class.badge-danger]="d.overview.health === 'unhealthy'" [class.badge-warning]="d.overview.health !== 'healthy' && d.overview.health !== 'unhealthy'">{{ d.overview.health }}</span></p></div>
          <div class="card min-w-0 p-4"><p class="flex items-center gap-1.5 text-xs c-muted"><kiban-icon name="box" [size]="12" /> Installed</p><p class="mt-1 text-sm font-medium kb-text">{{ d.overview.installedAt | slice:0:10 }}</p></div>
        </section>

        @if (presenter.errors(d).length > 0) {
          <section class="card min-w-0 p-4 border-danger/40">
            ...
            @for (error of presenter.errors(d); track error.state + error.lastError) {
              <div class="mt-3 grid grid-cols-[minmax(0,1fr)] gap-2 text-sm sm:grid-cols-3">
                ...
                <div><span class="block text-xs c-muted">Last error</span><span class="break-words kb-text">{{ error.lastError }}</span></div>
              </div>
            }
          </section>
        }
        ...
        <section class="grid grid-cols-[minmax(0,1fr)] gap-4 xl:grid-cols-2">
          <div class="card min-w-0 p-4">
            ...
            <div class="mt-3 grid grid-cols-[minmax(0,1fr)] gap-3 text-sm sm:grid-cols-2">
          ...
          <div class="card min-w-0 p-4">
            ...
            <div class="mt-3 space-y-2">
              @for (item of presenter.activity(d); track item.label + item.value) {
                <div class="flex flex-col items-start gap-1 rounded-lg border kb-border px-3 py-2 text-sm sm:flex-row sm:items-center sm:justify-between">
                  <span class="c-muted">{{ item.label }}</span>
                  <span class="break-words kb-text">{{ readableDate(item.value) || item.value }}</span>
                </div>
              }
            </div>
          </div>
          ...
          @if (d.accessPoints.length === 0) {
            ...
            <div class="mt-3 space-y-4">
              @if (presenter.webAccessPoints(d.accessPoints).length > 0) {
                <div class="min-w-0 rounded-xl border kb-border p-3">
                  ...
                  <div class="mt-3 flex flex-col gap-2 sm:flex-row">
                    <input class="input min-w-0 flex-1" type="text" placeholder="service.example.com" [ngModel]="serviceDomain()" (ngModelChange)="onServiceDomainChange($event)" [disabled]="!!actionInProgress() || serviceDomainSaving()" />
                  ...
                  @if (isLocalhostDomain()) {
                    ...
                    <p class="mt-2 text-xs c-muted">DNS required: <code class="break-all kb-text">A {{ serviceDomain() }} → your server IP</code>, or route this hostname through your tunnel provider.</p>
                  }
                ...
                <div>
                  ...
                  <div class="mt-2 grid grid-cols-[minmax(0,1fr)] gap-2 lg:grid-cols-2">
                    @for (ap of presenter.webAccessPoints(d.accessPoints); track ap.name + ap.port) {
                      <div class="flex flex-col gap-2 items-start sm:items-center rounded-lg border kb-border px-3 py-2 sm:flex-row sm:justify-between">
                        <a class="min-w-0 break-all font-mono text-xs kb-text hover:c-text sm:truncate" [href]="webUrl(ap)" target="_blank">{{ webUrl(ap) }}</a>
              ...
              @if (presenter.databaseAccessPoints(d.accessPoints).length > 0) {
                <div>
                  ...
                  <div class="mt-2 grid grid-cols-[minmax(0,1fr)] gap-3 lg:grid-cols-2">
                    @for (ap of presenter.databaseAccessPoints(d.accessPoints); track ap.name + ap.kind + ap.port; let i = $index) {
                      <article class="min-w-0 rounded-xl border kb-border p-3">
                        <h4 class="text-sm font-medium kb-text">{{ ap.name }}</h4>
                        <div class="mt-3 space-y-2">
                          @for (field of presenter.credentialFieldsFor(ap); track field.label) {
                            <div class="flex flex-col gap-2 items-start sm:items-center rounded-lg border kb-border px-3 py-2 sm:flex-row sm:justify-between">
                              <div class="min-w-0"><span class="block text-[10px] c-muted">{{ field.label }}</span><span class="block break-all text-xs font-mono kb-text sm:truncate">{{ field.secret ? presenter.displaySecret(field.value, visibleSecrets().has(i + ':' + field.label)) : field.value }}</span></div>
                        ...
                        @if (presenter.connectionStringFieldFor(ap); as connectionString) {
                          <div class="mt-3">
                            ...
                            <div class="mt-2 flex items-center justify-between gap-3 rounded-lg border kb-border px-3 py-2">
                              <div class="min-w-0"><span class="block break-all text-xs font-mono kb-text sm:truncate">{{ connectionString.secret ? presenter.displaySecret(connectionString.value, visibleSecrets().has(i + ':Connection String')) : connectionString.value }}</span></div>
                              <div class="flex gap-1">
        ...
        <section class="grid grid-cols-[minmax(0,1fr)] gap-4 xl:grid-cols-2">
          <div class="card min-w-0 p-4">
            ...
            <div class="mt-3 space-y-2">
              @for (c of presenter.containers(d); track c.id) {
                <div class="min-w-0 rounded-lg border kb-border p-3 text-sm">
                  <div class="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                    <span class="break-all font-medium kb-text sm:truncate">{{ c.name }}</span>
                  ...
                  <p class="mt-1 break-all font-mono text-xs c-muted sm:truncate">{{ c.image }}</p>
          ...
          <div class="xl:col-span-2 grid grid-cols-[minmax(0,1fr)] gap-4 xl:grid-cols-2">
            <!-- Terminal -->
            <div class="card min-w-0 overflow-hidden p-0 flex flex-col min-h-80 max-h-96">
            ...
            <!-- Logs -->
            <div class="card min-w-0 p-4 flex flex-col min-h-80 max-h-96">
              <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <h2 class="flex items-center gap-2 text-sm font-semibold kb-text"><kiban-icon name="logs" [size]="14" /> Logs</h2>
                <div class="flex flex-wrap items-center gap-1">
                  <select class="input h-8 min-w-0 flex-1 py-1 text-xs sm:w-36 sm:flex-none" [ngModel]="selectedLogContainer()" (ngModelChange)="selectedLogContainer.set($event)">
              ...
              <pre class="mt-3 min-h-0 min-w-0 flex-1 overflow-y-auto whitespace-pre-wrap break-words rounded-lg border kb-border p-3 text-xs c-muted">{{ displayedLogs(d) || 'No logs yet.' }}</pre>
            </div>
          </div>
        </section>

        <section class="card min-w-0 border-danger/30 p-4">
