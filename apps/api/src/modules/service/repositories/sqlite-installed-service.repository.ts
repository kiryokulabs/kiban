...
interface InstalledServiceRow extends SqliteRow { readonly id: string; readonly environment_id: string; readonly service_id: string; readonly name: string; readonly status: string; readonly configuration_json: string; readonly created_at: number; readonly updated_at: number; }
...
export class SqliteInstalledServiceRepository implements InstalledServiceRepository {
  ...
  public async create(input: Omit<InstalledService, 'id' | 'createdAt' | 'updatedAt'>): Promise<InstalledService> {
    ...
    await this.database.run('INSERT INTO installed_services (id, environment_id, service_id, name, status, configuration_json, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', [service.id, service.environmentId, service.serviceId, service.name, service.status, JSON.stringify(service.configuration), service.createdAt, service.updatedAt]);
    return service;
  }
  ...
  public async updateStatus(id: string, status: InstalledServiceStatus): Promise<InstalledService | null> {
    await this.database.run('UPDATE installed_services SET status = ?, updated_at = ? WHERE id = ?', [status, new Date(), id]);
  ...
  private toInstalledService(row: InstalledServiceRow): InstalledService {
    return { id: row.id, environmentId: row.environment_id, serviceId: row.service_id, name: row.name, status: row.status as InstalledServiceStatus, configuration: JSON.parse(row.configuration_json) as Readonly<Record<string, unknown>>, createdAt: new Date(row.created_at), updatedAt: new Date(row.updated_at) };
  }
}
